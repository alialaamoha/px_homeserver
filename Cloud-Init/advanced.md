...
```bash
     sudo qm create 8001 --name "ubuntu-2204-cloudinit-template" --ostype l26 \
        --memory 1024 \
        --agent 1 \
        --bios ovmf --machine q35 --efidisk0 trunk:0,pre-enrolled-keys=0 \
...
```bash
    qm importdisk 8001 jammy-server-cloudimg-amd64.img trunk
    qm set 8001 --scsihw virtio-scsi-pci --virtio0 trunk:vm-8001-disk-1,discard=on
    sudo qm set 8001 --boot order=virtio0
    sudo qm set 8001 --ide2 trunk:cloudinit
```
